...
class WebCatalogueHelperService implements IWebCatalogueHelperService {
  ...
  async getWebCatalogueDetails(
    ...
    try {
      ...
      storeProducts = await this.webCatalogueRepository.filterWebCatalogues(
        storeCatalogue,
  ...
  async filterHighlightsByIds(storeCatalogue: any) {
    ...
    if (storeCatalogue?.highlightIds?.length > 0) {
      let highlightsIds = storeCatalogue.highlightIds
        ...
        : [];
      let productList = await this.webCatalogueRepository.filterWebCatalogues({
        entityInternalId,
        productIds,
...
export default WebCatalogueHelperService;
