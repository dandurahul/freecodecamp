...
@injectable()
class IosCatalogueHelperService implements IIosCatalogueHelperService {
  @inject(ContainerTypes.IosCatalogueRepository)
  private iosCatalogueRepository!: IIosCatalogueRepository;

  async getIosCatalogueDetails(
    ...
    try {
      ...
      let productIdsData: any =
        await this.iosCatalogueRepository.filterIosCatalogues(
          storeCatalogue,
      ...
      storeProducts = await this.iosCatalogueRepository.filterIosCatalogues(
        storeCatalogue,
  ...
  async filterHighlightsByIds(storeCatalogue: any) {
    ...
    if (storeCatalogue?.highlightIds?.length > 0) {
      let highlightsIds = storeCatalogue.highlightIds
        ...
        : [];
      let productList = await this.iosCatalogueRepository.filterIosCatalogues({
        entityInternalId,
        productIds,
...
export default IosCatalogueHelperService;
